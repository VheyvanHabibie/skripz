...
@section('content')
    <div class="container-fluid">
                        ...
                        <div class="card shadow mb-4 h-100">
                            <div class="card-body text-center">
                                ...
                                <div class="card-text mb-2">
                                    <strong class="card-title my-0">{{ $item->nama_mitra }}</strong>
                                </div>
                            </div>
                                            ...
                                            <div class="dropdown-menu m-2">
                                                <button class="dropdown-item" data-toggle="modal" data-target="#editModal{{ $item->id }}"
                                                    data-whatever="@mdo" type="button"><i
                                                        class="fe fe-edit fe-12 mr-4"></i>Edit</button>
                                                <form id="deleteForm{{ $item->id }}">
                                                    {{-- <a class="dropdown-item" href="./detail.html"><i
                                                        class="fe fe-eye fe-12 mr-4"></i>Detail</a> --}}
                                                    <button class="dropdown-item" onclick="deleteMitra({{ $item->id }})"
        ...
        @else
            <h4 class="text-center font-weight-bold">Belum Ada Data Mitra</h4>
        @endif
    </div>
    @include('backend.mitra.create')
    @include('backend.mitra.edit')
    <script>
        function deleteMitra(itemId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('mitrapengguna.destroy', ':id') }}'.replace(":id", itemId),
                        type: 'DELETE',
                        headers: {
