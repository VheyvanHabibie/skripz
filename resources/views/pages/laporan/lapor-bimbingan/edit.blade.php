...
<div class="modal fade" id="editModal{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('laporan-bimbingan.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="modal-body">
                   <div class="form-group">
                        <label for="tanggal_bimbingan" class="required">Tanggal Bimbingan</label>
                        <input type="date" class="form-control" id="tanggal_bimbingan" name="tanggal_bimbingan" value="{{ $item->tanggal_bimbingan }}" required>
                    </div>
                    <div class="form-group">
                        <label for="topik_bahasan" class="required">Topik Bahasan</label>
                        <input type="text" class="form-control" id="topik_bahasan" name="topik_bahasan" value="{{ $item->topik_bahasan }}" required>
                    </div>
                    <div class="form-group">
                        <label for="hasil_bimbingan" class="required">Hasil Bimbingan</label>
                        <input type="text" class="form-control" id="hasil_bimbingan" name="hasil_bimbingan" value="{{ $item->hasil_bimbingan }}" required>
                    </div>
                    <div class="form-group">
                        <label for="saran_pembimbing" class="required">Saran Pembimbing</label>
                        <input type="text" class="form-control" id="saran_pembimbing" name="saran_pembimbing" value="{{ $item->saran_pembimbing }}" required>
                    </div>
                    <div class="form-group">
