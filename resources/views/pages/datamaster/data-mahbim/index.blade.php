...
@section('content')
                ...
                <div class="row">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Mahasiswa Bimbingan</li>
                        </ol>
                    </nav>
                    <div class="col-md-12">
                        <div class="card shadow-smooth custom-card">
                            <div class="card-body">
                                @can('akses tambah-mahbim')
                                        ...
                                        <tbody class="text-center">
                                            @foreach ($mahbim as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->mahasiswa->no_telepon }}</td>
                                                    <td>
                                                        <form action="{{ route('mahasiswa-bimbingan.destroy', $item->id) }}"
                                                            method="POST" id="Hapus{{ $item->id }}">
                                                            @method('DELETE')
                                                            @csrf
                                                            @can('akses hapus-mahbim')
                                                                <button class="btn btn-outline-danger" type="submit"
                                                                    id="Hapus"
                                                                    onclick="deleteActivity({{ $item->id }})"><i
                                                                        class="fe fe-trash fe-12"></i></button>
                                                            @endcan
                                                            @can('akses edit-mahbim')
    ...
    @include('pages.datamaster.data-mahbim.create')
    @include('pages.datamaster.data-mahbim.edit')
@endsection
