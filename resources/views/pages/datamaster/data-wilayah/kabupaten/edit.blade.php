                ...
                <form action="{{ route('kabupaten.update', $item->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="provinsi_id{{ $item->id }}" name="provinsi_id" required>
                                <option value="">Pilih Provinsi</option>
                                @foreach($provinsi as $prov)
                                    <option value="{{ $prov->id }}" {{ $item->provinsi_id == $prov->id ? 'selected' : '' }}>
                                        {{ $prov->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="full_code{{ $item->id }}" name="full_code" value="{{ $item->full_code }}" required>
                        </div>
                        <div class="form-group">
                            <label for="type{{ $item->id }}" class="required">Tipe</label>
                            <input type="text" class="form-control" id="type{{ $item->id }}" name="type" value="{{ $item->type }}" required>
                        </div>
                    </div>
