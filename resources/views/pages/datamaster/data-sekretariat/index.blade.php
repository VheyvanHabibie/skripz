...
@section('content')
                ...
                <div class="row">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Sekretariat</li>
                        </ol>
                    </nav>
                    <div class="col-md-12">
                        <div class="card shadow-smooth custom-card">
                            <div class="card-body">
                                @can('akses tambah-sekretariat')
                                    ...
                                    <a class="btn mb-0 btn-info mb-3 mr-2" type="button"
                                        href="{{ asset('assets/file/Template_Form_Data_Sekretariat.xlsx') }}" download>
                                        ...
                                        <tbody>
                                            @foreach ($sekretariat as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->nama_sekretariat }}</td>
                                                    <td>{{ $item->nip }}</td>
                                                    {{-- <td>{{ $item->jabatan->nama_jabatan }}</td> --}}
                                                    <td>{{ $item->email }}</td>
                                                    <td>{{ $item->no_telepon }}</td>
                                                    <td><a href="#!" class="avatar avatar-sm">
                                                            <img alt="foto{{ $item->id }}"
                                                                class="avatar-img rounded-circle"
                                                                src="{{ asset($item->foto) }}">
                                                        </a></td>
                                                    <td class="text-center">
                                                        <form action="{{ route('sekretariat.destroy', $item->id) }}"
                                                            method="POST" id="Hapus{{ $item->id }}">
                                                            @method('DELETE')
                                                            @csrf
                                                            @can('akses hapus-sekretariat')
                                                                <button class="btn btn-outline-danger" type="submit"
                                                                    id="Hapus"
                                                                    onclick="deleteActivity({{ $item->id }})"><i
                                                                        class="fe fe-trash fe-12 "></i></button>
                                                            @endcan
                                                            @can('akses edit-sekretariat')
    ...
    @include('pages.datamaster.data-sekretariat.create')
    @include('pages.datamaster.data-sekretariat.edit')
    @include('pages.datamaster.data-sekretariat.import')
    <script>
