...
@section('content')
                ...
                <div class="row">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Sumber Referensi</li>
                        </ol>
                    </nav>
                    <div class="col-md-12">
                        <div class="card shadow-smooth custom-card">
                            <div class="card-body">
                                @can('akses tambah-sumber-referensi')
                                        ...
                                        <tbody>
                                            @foreach ($sumber as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->sumber_referensi }}</td>
                                                    <td>
                                                        <form action="{{ route('sumber-referensi.destroy', $item->id) }}"
                                                            method="POST" id="Hapus{{ $item->id }}">
                                                            @method('DELETE')
                                                            @csrf
                                                            @can('akses hapus-sumber-referensi')
                                                                <button class="btn btn-outline-danger" id="Hapus"
                                                                    onclick="deleteActivity({{ $item->id }})"><i
                                                                        class="fe fe-trash fe-12"></i></button>
                                                            @endcan
                                                            @can('akses edit-sumber-referensi')
    ...
    @include('pages.datamaster.data-sumber.create')
    @include('pages.datamaster.data-sumber.edit')
@endsection
